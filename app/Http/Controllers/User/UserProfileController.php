...
class UserProfileController extends Controller
{
    ...
    public function saveSection(Request $request)
    {
        ...
        // Define per-section field groups
        $sectionFields = [
            'section1' => [
                // User fields
                'first_name', 'last_name', 'middle_name', 'suffix', 'birth_date', 'place_of_birth', 'sex',
                // Profile fields
                'civil_status', 'height', 'weight', 'blood_type', 'gsis_id_no', 'pagibig_id_no', 'philhealth_no', 'sss_no', 'tin_no', 'agency_employee_no', 'citizenship', 'dual_country_dropdown', 'dual_country', 'perm_house_unit_no', 'perm_street', 'perm_barangay', 'perm_city_municipality', 'perm_province', 'perm_zipcode', 'res_house_unit_no', 'res_street', 'res_barangay', 'res_city_municipality', 'res_province', 'res_zipcode'
            ],
            'section2' => [
                'spouse_surname', 'spouse_first_name', 'spouse_middle_name', 'spouse_name_extension', 'spouse_occupation', 'spouse_employer', 'spouse_business_address', 'spouse_telephone_no',
                'father_surname', 'father_first_name', 'father_middle_name', 'father_name_extension',
                'mother_surname', 'mother_first_name', 'mother_middle_name',
                'children_names', 'children_birthdates'
            ],
        ...
        if (!array_key_exists($section, $sectionFields)) {
            return response()->json(['success' => false, 'error' => 'Invalid section.']);
        }

        $data = $request->only($sectionFields[$section]);

        // Separate user and profile fields for section1
        if ($section === 'section1') {
            $userFields = ['first_name', 'last_name', 'middle_name', 'suffix', 'birth_date', 'place_of_birth', 'sex'];
        ...
        if ($section === 'section2') {
            ...
            unset($data['children_names'], $data['children_birthdates']);
        }
        ...
        return response()->json(['success' => true, 'message' => ucfirst(str_replace('section', 'Section ', $section)) . ' saved.']);
    }
